...
# random param check .. not useful. Try to find if I can set arbitrary param and then read it. 
...
def get_type(drone):
    while True:
        try:
            ...
            #print(f"type of msg received is: {type(msg)}")
            # Extract vehicle type from the HEARTBEAT message
            if msg:
                vehicle_type = msg.type
                
                mode = mavutil.mode_string_v10(msg)
                
                reported_mode = msg.custom_mode
                print(f"Reported Flight Mode: {reported_mode}")
                #system_status = mavutil.mavlink_system_status_string(msg.system_status)
                system_status = msg.system_status
                # print(f"received ACK: {mavutil.mavlink.enums['MAV_RESULT'][ack_msg['result']].description}")
                
                print(f"Vehicle TYPE: {mavutil.mavlink.enums['MAV_TYPE'][vehicle_type].description}, ENUM value: {vehicle_type}")
                print(f"mode: {mode}, status: {system_status}")
                break
                #print(f"again: {dialect.MAVLink_heartbeat_message(11).name}")
