...
Takes an array like th eone outputed by GetScan (see indicative outputs)
and then returns the distance measurements that are "close"
proximity between points is determined by:
        Angle between them
        indicated distance from the sensor
        strength of the returning signal
it then outputs an array of processed data, in the form of a point object with some
extra params.

TODO: refine this so its easier to understand/explain.
