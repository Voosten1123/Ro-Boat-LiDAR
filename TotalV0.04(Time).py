...
while itera<100:
    ...
    dataalpba = get_scan()
    scanexit = time()
    sum1 = sum1 + (scanexit - scanentry)

    processentry = time()
    data1 = process_scan(dataalpba, 1.5, 3, 14)
    processexit = time()
    sum2 = sum2 + (processexit-processentry)

    data2 = process_scan(get_scan(), 1.5, 3, 14)

    compareentry = time()
    dataold = Compare_Scans(data1, data2, 3, 15, 120, 150)
    compareexit = time()
    sum3 = sum3 + (compareexit-compareentry)
    itera +=1
sum1 = sum1/itera
sum2 = sum2/itera
sum3 = sum3/itera
print('-----------------------------------')
print('Get Scan average time:', sum1)
print('Process Scan average time:', sum2)
print('Compare Scans average time:', sum3)
print('-----------------------------------')
print('Cycles:', itera)