...
def lidar_check():
    try:
        vehicle.groundspeed = 1 #set speed to low so we can get better detections. measured in m/s
        scan1 = process_scan(get_scan(), 0.15, 3, 12)
        scan2 = process_scan(get_scan(), 0.15, 3, 12)
...
def turning_mode_enable(buoy, angle_thres, distance_thres):
    '''
    checks to see: that we're looking at the buoy the camera says we are. that the distance is below 10m. it then issues a command to move to the next waypoint
    ...
    try:
       ...
       i = 0
       while i < len(scan1):
           if scan1[i][1] - buoy[2] < angle_thres:    #if the detected object has a mean angle near the one given by the buoy object, then:
               if scan1[i][2] - buoy[1] < distance_thres:    #if their distances are near enough
                   if scan1[i][2] < 10000:
...
def camera_check_speed(detections, distance):
    '''
    a simpler camera check which skips the lidar checking for the speed race
    it also includes the finish line logic, which we could copy into other races should we need to
    '''
    try:
        finishline = 0
        ldet = len(detections)
...
def speed_race():
    try:
        while True:
            camera_check_speed(Detection.get_detections(), 10000)
try:
    i = 0
    while True:
