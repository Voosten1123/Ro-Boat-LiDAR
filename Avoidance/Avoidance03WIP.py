        ...
        if ldet>0:      #only check cam if there's a detected
            if ldet == 1:       #if its only 1 detection
                if detections[0][2] <10 and detections[0][2] >350:  #is it in front of me
                    if detections[0][1] < 10000:            #is it a possible threat
                    ...
                    if detections[0][0] = "largebuoy":      #its a large buoy but its not in front of me
                        if is_the_next_waypoint(detections[0]):  #if its the waypoint I'm currently heading to
                            target = []
                            target = destination_location(CurrLat(), CurrLon(), detections[0][1]-15, detections[0][2])
                            targetLat = target[0]
                            targetLon = target[1]
                            target = []
                            send_to(targetLat, targetLon, 0,)
                            #alternatively
                            manuever(-detections[0][2])
